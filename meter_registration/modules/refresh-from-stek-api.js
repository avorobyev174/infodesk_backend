...
module.exports = class refreshFromStekApi {
	constructor(app, module_name) {
		...
		app.get(`/api/${ module_name }/refresh-data-from-stek/`, (apiReq, apiRes) => {
				showRequestInfoAndTime(`Регистрация счетчиков: запрос на обновление данных из стека для пирамиды`)
				...
				const query = `select * from meters where in_pyramid = 1 order by random() limit 10000`
		...
		app.put(`/api/${ module_name }/save-refresh-meter-data-from-stek/:id`, (apiReq, apiRes) => {
